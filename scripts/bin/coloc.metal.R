...
for (y in list.files(path=args[1],pattern=args[2]))tryCatch({
    ...
    my.res <- coloc.abf(dataset1=list(beta=df$BETA_mvp, varbeta=df$SE_mvp, type="quant"), dataset2=list(beta=df$BETA_gtex, varbeta=df$SE_gtex, N=df[1,"OBS_CT_gtex"],type="quant"),MAF=df$maf)
    ...
    my.res <- coloc.abf(dataset1=list(pvalues=df$P_mvp, type="quant"), dataset2=list(pvalues=df$P_gtex, N=df[1,"OBS_CT_gtex"],type="quant"), MAF=df$maf)
