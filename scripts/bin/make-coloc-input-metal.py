...
def make_coloc_input(gtex_df,mvp_df,out):
    ...
    mvp["BETA"]=mvp["Effect"].astype(float)
    mvp["SE"]=mvp["StdErr"].astype(float)
    mvp["P"]=mvp["P-value"].astype(float)
    mvp=mvp[["MarkerName","BETA","SE","P"]]
    
    mvp["SNP"]=mvp["MarkerName"].str.rsplit(":",2).str[0]
